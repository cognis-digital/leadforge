...
**leadforge** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `leadforge` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
leadforge ... --format json | cognis-connect emit --to sigma   # Sigma rules
leadforge ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
leadforge ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `leadforge` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="leadforge") for rec in records]   # records = your JSON output
