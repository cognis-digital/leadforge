...
How **leadforge** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `leadforge` lives in the **MCP / agent security** cluster.
...
| from | relation | to |
|---|---|---|
| `leadforge` | JSON findings compose with | [`mcpharden`](https://github.com/cognis-digital/mcpharden), [`c2detect`](https://github.com/cognis-digital/c2detect), [`agentmap`](https://github.com/cognis-digital/agentmap) |
| `leadforge` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `leadforge` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `leadforge` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the mcp / agent security cluster.** `leadforge` output feeds the next tool:
```bash
leadforge ... --format json > out.json          # this tool's findings (see `leadforge --help`)
...
leadforge ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
leadforge ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
leadforge ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `leadforge`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `leadforge` slots
